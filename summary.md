...
- 저녁식사 후 방송은 다시 시작 전 대기 화면으로 전환되어 57:00 카운트다운과 “잠시 후 마비노기 라이브 방송이 시작됩니다” 안내가 표시됐다.
...
| 19:03~19:05 | 음성/Q&A | 디렉터가 유저 출신이라는 이미지와 실제 업데이트로 인한 신뢰 저하를 지적하며, 운영진이 유저가 게임을 어떻게 사랑하는지/어떤 시각을 갖는지 이해해야 한다는 의견이 제기됨 → 답변은 다음 구간에서 이어짐 | `transcripts/chunk_000548.txt`~`transcripts/chunk_000551.txt` |
...
| 유저 출신 디렉터 이미지와 신뢰 저하 | 디렉터가 유저 출신이라는 이미지와 실제 업데이트로 인한 신뢰 저하를 지적하며, 운영진이 유저가 게임을 어떻게 사랑하는지 이해해야 한다는 의견 | 답변은 다음 구간에서 이어짐 | `transcripts/chunk_000548.txt`~`transcripts/chunk_000551.txt` |
...
- 후반부에는 운영 커뮤니케이션 미숙, 브리레 플레이 경험 공개 요청, 공개 플레이 영상과 사례 설명 요구가 이어졌고 답변은 계속 진행 중이었다.
- 공개 플레이 영상은 정형화 우려 때문에 너무 이르게는 어렵지만 고리아스 던전 공개 시점에 맞춰 공개하겠다고 했고, 오늘 나온 내용은 별도로 정리해 마비노기 커넥트/테스트 서버 일정으로 공유하겠다고 답했다.
- 디렉터가 유저 출신이라는 이미지가 있었지만 실제 업데이트 경험으로 신뢰가 흔들렸다는 의견도 나왔고, 운영진이 유저가 게임을 어떻게 사랑하는지 이해해야 한다는 요청이 이어졌다.
- MC 김효진 아나운서 진행 화면으로 전환된 뒤, 저녁식사 후 20시에 재개하겠다는 공지가 나왔고, 재개 직전에는 57:00 카운트다운의 시작 전 대기 화면이 확인됐다.
