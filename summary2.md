...
- 최신 캡처 1148~1164는 ON AIR 패널과 객석 Q&A가 이어졌고, 1129에서는 콘텐츠 리더 강민석 명패가 확인됐다.
...
- 249~388번 구간에서는 악기 연주 콘텐츠의 보관/검색/개편 요청, 교역 보상 6단계 표기 오해와 두카트 소모처 방향, 일반 교역 이동 불편, 낭만 농장·달빛섬 편의성 개편, 미니어처 검색, 파견 잠금 오류, 지형·배경 변경 문의, 낭만 농장 방명록 버그, 주크박스 동작 범위, 농장 통합 요청, 세이크리드 가드/세인트바드 개편 논쟁과 참여 비중 그래프 설명, 세가 딜/생존력 조정과 테스트 서버 연장, 유저 신뢰 회복 방향까지 생활·전투형 편의와 밸런스 요청이 연달아 이어졌다. 교역 보상은 하반기 목표로 더 자연스럽게 손볼 예정이고, 두카트 소모처는 전투 성장을 생활에 묶지 않는 선에서 경제 영향까지 고려해 검토하겠다고 했다.
...
- 최신 캡처 1148~1164에서는 객석 질의 장면과 ON AIR 패널 진행이 이어졌고, 별도 슬라이드 없이 토크/Q&A가 반복됐다.
- 현재도 진행 중이며, 최근 반영분은 audio transcript 388번까지와 최신 패널 캡처를 기준으로 정리했다.
