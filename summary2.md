...
- 최신 캡처 1660~1752는 콘텐츠 리더 강민석 이름표가 보이는 ON AIR 패널/현장 Q&A 장면과 서가/세바 개편의 세인트 바드 질문 슬라이드로 이어졌고, 세인트바드 지향점, 링크 보너스, 타이틀, 유물, 신규 스킬, 다기능 무기 아이디어, 무기군 간격, 툴팁 확장, 딜링 스킬 밸런스 논의가 계속됐다. 1427은 00:45 카운트다운이 보이는 마비노기 커넥트 LIVE 대기 화면이었다.
...
- 554~587번 구간에서는 마이크처럼 힐링과 연주를 함께 시전할 수 있는 다기능 무기 아이디어가 제안됐고, 다음 무기군부터 이중 부담을 줄이는 방향과 2차 타이틀·성수작 보정, 나이트 브링어/소울 원드 간 간극, 스킬 툴팁 확장, 은파의 세레 디버프 표시 방식, 네다섯 줄을 맞춰야 하는 무기 세공 부담, 하위 던전 솔로 플레이, 힐링 원드 세공 파밍, 딜링 스킬 밸런스까지 함께 고민하겠다는 답이 나왔다.
## 주요 화면 캡처
...
- 마지막에는 다기능 무기와 성수작/2차 타이틀 보정 논의까지 이어지며, 세인트바드의 이중 부담을 줄이는 다음 무기군 설계와 나이트 브링어/소울 원드 간 간극 조정, 스킬 툴팁 확장, 무기 세공 부담 완화, 하위 던전 솔로 플레이, 힐링 원드 세공 파밍, 딜링 스킬 밸런스가 함께 검토됐다.
